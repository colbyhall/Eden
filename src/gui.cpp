...
bool gui_button(GUI_ID id, const ch::String& s, f32 x0, f32 y0, f32 x1, f32 y1) {
	bool result = false;
	if (active_id == id) {
		if (!the_input_state.lmb_down && the_input_state.lmb_was_down) {
			if (hovered_id == id) {
				result = true;
			}
			active_id = zero_id;
		}
	} else if (hovered_id == id && the_input_state.lmb_down && !the_input_state.lmb_was_down) {
		set_active_id(id);
	}

	if (is_point_in_rect(the_input_state.current_mouse_position, x0, y0, x1, y1)) {
		hovered_id = id;
